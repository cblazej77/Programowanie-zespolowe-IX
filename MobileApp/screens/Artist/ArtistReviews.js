...
const ArtistReviews = ({ data, input, setInput }) => {
  return (
    <FlatList
      style={{ margin: 2 }}
      data={DATA_reviews}
      ...
      ItemSeparatorComponent={
        <View style={{ width: '100%', height: 1, backgroundColor: '#e4dfe1', marginVertical: 5 }} />
      }
      renderItem={({ item }) => {
        return (
          <View>
            ...
            <RegularText numberOfLines={2} style={{ fontSize: 13, marginVertical: 5 }}>
              {item.review_text}
            </RegularText>
          </View>
        );
